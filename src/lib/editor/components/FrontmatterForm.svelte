<script lang="ts">
  ...
  import { parseDoc, composeDoc } from '$lib/editor/frontmatter';
  ...
  // `parseDoc` normalizes Date values to `YYYY-MM-DD` strings, but guard
  // against anything odd still slipping through (e.g. `date: 20250202`
  // as a number, or a full ISO datetime) and reject anything that isn't
  // the exact `<input type="date">` format.
  const date = $derived(toDateInputValue(fm.date));
  const updated = $derived(toDateInputValue(fm.updated));
