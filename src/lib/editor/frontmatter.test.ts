...
describe("date handling", () => {
  it("normalizes unquoted YYYY-MM-DD to string at parse time", () => {
    // Without normalization js-yaml would parse this as Date objects,
    // which then coerce to the wrong shape in `<input type=\"date\">`.
    const { fm } = parseDoc(`---
date: 2025-02-02
updated: 2025-06-25
---
`);
    expect(fm.date).toBe("2025-02-02");
    expect(fm.updated).toBe("2025-06-25");
  });

  it("round-trips unquoted dates without drifting the on-disk format", () => {
    // The original .svx on disk uses unquoted YYYY-MM-DD; an edit of an
    // unrelated field must not rewrite dates to ISO-with-Z or quoted form.
    const src = `---
title: Hello
date: 2025-02-02
updated: 2025-06-25
...
body`;
    const { fm, body } = parseDoc(src);
    const out = composeDoc({ ...fm, title: "Hello edited" }, body);
    expect(out).toContain("date: 2025-02-02");
    expect(out).toContain("updated: 2025-06-25");
    // Specifically: no ISO suffix, no quoting
